# -*- coding: utf-8 -*-
"""Kodi directory/plugin entry point for the ClassicalRadio add-on."""
...
def log(msg, level=xbmc.LOGINFO):
    xbmc.log(f"[plugin.audio.classicalradio] {msg}", level)
...
def run():
    ...
    try:
        ...
        xbmcgui.Dialog().ok("ClassicalRadio", str(exc))
        try:
            xbmcplugin.endOfDirectory(
        ...
        xbmcgui.Dialog().ok("ClassicalRadio", t(32031))
        try:
            xbmcplugin.endOfDirectory(
                HANDLE, succeeded=False, cacheToDisc=False
            )
